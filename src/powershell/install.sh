...
install_using_github() {
  ...
  ln -sf "${powershell_target_path}/pwsh" /usr/local/bin/pwsh
  rm -rf /tmp/pwsh
}
...
if [ ${#POWERSHELL_MODULES[@]} -gt 0 ]; then
  ...
  for i in "${modules[@]}"; do
    ...
    pwsh -Command "& {Install-Module -Name ${i} -Repository PSGallery -AllowClobber -Force -Scope AllUsers}" || continue
  done
fi
